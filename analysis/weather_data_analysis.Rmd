...
```{r}
ce_temps = read.table("C:/Users/aclou/Documents/Projects/SCBs_Exploration/data/ce_ssn_temp_mean_raw.txt", header = TRUE)
...
- Is my periodogram approach correct? Why is there mention of a the spectral density function = $\sigma^2/(2\pi)$, so should I multiply the periodogram by $2\pi$? <br> 
...
1/(sum(smooth_1$coef^2) + sum(smooth_1$coef[-1]^2))/length(ce_temps_yr_ts)

smooth_2 = kernel("modified.daniell", c(18,18))
1/(sum(smooth_2$coef^2) + sum(smooth_2$coef[-1]^2))/length(ce_temps_yr_ts)
...
Final bandwidths to try: `r paste(round(c(b_final_1, b_final_2, b_final_3, b_final_4), 3), collapse = ", ")`. 
...
Notes/Q's: <br>
- why do empirical coverage rates seem quite low across all 95% SCBs? <br>
- This even applies for bandwidths as small as 0.02 (around 7 years), which only has empirical coverage around 93%. <br>
- How short would Ruppert (1995) optimal bandwidth be? Even shorter than 0.02? <br>
- Is our sigma estimated with spectral density and periodogram too low? Should we use Equation (25) instead? <br> 
