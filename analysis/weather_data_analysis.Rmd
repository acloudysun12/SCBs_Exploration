...
k_final_0 = calc_k_final(years, ce_temps_yr_ts, sigmas$hat_3)$k_final
...
k_final_q1 = calc_k_final(years, ce_temps_q1_ts, sig_hat_q1)$k_final
k_final_q2 = calc_k_final(years, ce_temps_q2_ts, sig_hat_q2)$k_final
k_final_q3 = calc_k_final(years, ce_temps_q3_ts, sig_hat_q3)$k_final
k_final_q4 = calc_k_final(years, ce_temps_q4_ts, sig_hat_q4)$k_final
...
The optimal bandwidth for spring (Qtr2) is `r b_final_q2` (Ruppert's automatic bandwidth selector = `r round(dpill(years, ce_temps_q2_ts)/length(years), 3)`), while the optimal bandwidth for summer (Qtr3) is `r b_final_q3` (Ruppert bandwidth = `r round(dpill(years, ce_temps_q3_ts)/length(years), 3)`). TO RESEARCH -- WHY IS THIS THE CASE? <br>
The optimal bandwidth for summer (Qtr3) is larger than 0.2, which we have shown in the simulation study to lower the nominal coverage due to higher bandwidths leading to higher bias. <br>
...
plot(ce_temps_q1_ts, main = paste("Q1 temps with LL est. and SCB for optimal b = ", round(k_final_q1/360, 3)))
...
plot(ce_temps_q2_ts, main = paste("Q2 temps with LL est. and SCB for optimal b = ", round(k_final_q2/360, 3)))
...
plot(ce_temps_q3_ts, main = paste("Q3 temps with LL est. and SCB for optimal b = ", round(k_final_q3/360, 3)))
...
plot(ce_temps_q4_ts, main = paste("Q4 temps with LL est. and SCB for optimal b = ", round(k_final_q4/360, 3)))
...
We observe from the plots that the local linear estiamtes and correponding 95% SCBs for the temperatures series broken out by quarter are not similar. This again is likely due to the large range of bandwidths (0.05 to 0.20) selected earlier. <br>
